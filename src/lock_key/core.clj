...
(defn encrypt
  ...
  value           - (String/byte[]) the value to encrypt. Raises an AssertionError
                    if value is an invalid type.
  encryption-key  - (String) the key with which to encrypt value with. Raises an
                    AssertionError if encryption-key is an invalid type or empty"
  ^bytes
  [value encryption-key]
  (assert (or (string? value)
               (private/byte-array? value)) "Argument [value] must be of type String or byte[]")
  (assert (and (string? encryption-key)
               (not (empty? encryption-key))) "Argument [encryption-key] must be a string and not empty")
...
(defn decrypt
  ...
  value          - (byte[]) the value to decrypt. Raises an AssertionError if value
                   is an invalid type
  encryption-key - (String) the key with which the value was encrypted. Raises an
                   AssertionError if encryption-key is an invalid type or empty"
  ^bytes
  [value encryption-key]
  (assert (private/byte-array? value) "Argument [value] must be of type byte[]")
  (assert (and (string? encryption-key)
               (not (empty? encryption-key))) "Argument [encryption-key] must be a string and not empty")
...
(defn decrypt-from-base64
  ...
  [value encryption-key]
  (assert (string? value) "Argument [value] must be of type String")
